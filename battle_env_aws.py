...
class game_map:
    ...
    def step(self,team,ship_type,ship_id,action):
        ...
        return self.get_rew(team),self.termi_cond()
    ...
    def get_rew(self,team):
        ...
        for i in range(len(self.team_split)):
            for j in range(self.team_split[i]):
                if j not in self.dead_ships:
                    if i==team:
                        rew+=self.map_dict[i][0][j]['health']
                    else:
                        rew-=self.map_dict[i][0][j]['health']
                    #k+=1
        return rew
    ...
    def get_vec(self):
        # returns a vector of states of all bots
        vec = np.zeros(sum(self.team_split)*6)
        k = 0
        for i in range(len(self.team_split)):
            for j in range(self.team_split[i]):
                vec[6*k] = self.map_dict[i][0][j]['theta']
                vec[6*k+1] = self.map_dict[i][0][j]['x']
                vec[6*k+2] = self.map_dict[i][0][j]['y']
                vec[6*k+3] = self.map_dict[i][0][j]['health']
                vec[6*k+4] = self.map_dict[i][0][j]['fuel']
                vec[6*k+5] = self.map_dict[i][0][j]['missiles']
                k+=1
        return vec
class nn():
    def __init__(self,state_space,action_space,layers = []):
        # have to pass only one long weight array top train 
        print('number of hidden layers is %d'%len(layers))
        self.w = []
        self.b = []
        self.layers = layers
        self.layers.insert(0,state_space)
        self.layers.append(action_space)
        self.num_arr = [0]
        for i in range(len(self.layers)-1):
            self.w.append(np.random.uniform(-5,5,size = (self.layers[i],self.layers[i+1])))
            self.b.append(np.random.uniform(-5,5,size = (1,self.layers[i+1])))
            self.num_arr.append(self.layers[i]*self.layers[i+1])
            self.num_arr.append(self.layers[i+1])
        self.num_arr = np.cumsum(self.num_arr)
        self.total_elem = self.num_arr[-1]
    def action(self,state):
        x = state.reshape((1,len(state)))
        for i in range(len(self.w)):
            x = np.matmul(x,self.w[i]) + self.b[i]
        return np.argmax(x,1)[0]
    def set_weights(self,w):
        assert len(w) == self.total_elem
        k = 0
        count = 0 # counter for reashaping
        for i in range(len(self.w)):
            self.w[i] = w[self.num_arr[k]:self.num_arr[k+1]]
            self.w[i] = self.w[i].reshape((self.layers[count],self.layers[count+1]))
            k+=1
            self.b[i] = w[self.num_arr[k]:self.num_arr[k+1]]
            self.b[i] = self.b[i].reshape([1,self.layers[count+1]])
            k+=1
            count+=1
        return
